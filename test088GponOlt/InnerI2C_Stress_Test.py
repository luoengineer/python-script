...
if 'OK' == chr(strCmdOut[0])+chr(strCmdOut[1]):
    ...
    print("Write APC lut, ok")
    f.write("Write APC lut, ok")
    ...
    cmdservdll.SuperCmdSer(strCmdIn, strCmdOut)
    if 'OK' == chr(strCmdOut[0]) + chr(strCmdOut[1]):
        print("Set APC auto mode, ok")
        f.write("Set APC auto mode, ok")
    else:
        print("Set APC auto mode, fail ")
        f.write("Set APC auto mode, fail ")
else:
    print("Write APC lut, fail")
    f.write("Write APC lut, fail")
#########################################################
...
if 'OK' == chr(strCmdOut[0])+chr(strCmdOut[1]):
    # TODO: first read back,then hold target,max,miin
    print("Write MOD Max lut, ok")
    f.write("Write MOD Max lut, ok")
    ...
    cmdservdll.SuperCmdSer(strCmdIn, strCmdOut)
    if 'OK' == chr(strCmdOut[0]) + chr(strCmdOut[1]):
        print("Set MOD Max auto mode, ok")
        f.write("Set MOD Max auto mode, ok")
    else:
        print("Set MOD Max auto mode, fail ")
        f.write("Set MOD Max auto mode, fail ")
else:
    print("Write MOD Max lut, fail")
    f.write("Write MOD Max lut, fail")
#########################################################
#           Write AER Lut and set auto mode
#########################################################
f.write('\n')
strCmdIn = create_string_buffer(b'MCU_SET_TABLE(LUT,6,0,0x5A,0xFE,0xA8,0x00,0x1E,0xFF,0xFC,0x6E,0x00,0x52)')
f.write(repr(strCmdIn.value)+'\n')
strCmdOutBuff = ctypes.c_ubyte*8
strCmdOut = strCmdOutBuff()
cmdservdll.SuperCmdSer(strCmdIn, strCmdOut)
if 'OK' == chr(strCmdOut[0])+chr(strCmdOut[1]):
    # TODO: first read back,then hold target,max,miin
    print("Write AER lut, ok")
    f.write("Write AER lut, ok")
    ...
    cmdservdll.SuperCmdSer(strCmdIn, strCmdOut)
    if 'OK' == chr(strCmdOut[0]) + chr(strCmdOut[1]):
        print("Set AER auto mode, ok")
        f.write("Set AER auto mode, ok")
    else:
        print("Set AER auto mode, fail ")
        f.write("Set AER auto mode, fail ")
else:
     print("Write AER lut, fail")
     f.write("Write AER lut, fail")
#########################################################
...
#########################################################
    if tempIndex < 0x60:
        ...
        print("\nGN25L96 AER auto mode,now tempIndex is {}, 25L96 A8 should adjust to '0x{:02X}'".format(tempIndex, int(
            tempIndex * (int('0xFC6E', 16)-65535) / 1000 + int('0x52', 16))))
        f.write("\nGN25L96 Mod Max auto mode,now tempIndex is {}, 25L96 A8 should adjust to '0x{:02X}'".format(tempIndex, int(
            tempIndex * (int('0xFC6E', 16)-65535) / 1000 + int('0x52', 16))))
    f.write("\n")
