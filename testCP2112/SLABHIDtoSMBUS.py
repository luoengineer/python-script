...
class HidSmbusDevice:
    ...
    def AddressReadRequest(self, address=2, count=64, offset_size=2, offset=b'\x00\x00'):
        buf = [0x00] * 16
        buf[0] = offset
        bufp = (ct.c_ubyte * 16)(*buf)
        _DLL.HidSmbus_AddressReadRequest(self.handle, address, count, offset_size, ct.byref(bufp))
...
def Test( DevIndex):
    ...
    try:
        ...
        slave_address = 0xA0
        read_length = 61
        total_read_length = 62
        reg_size = 1
        reg_offset = 2
        rev_buf = ct.c_ubyte * 256
        smb.AddressReadRequest(slave_address, read_length, reg_size, reg_offset)
        smb.TransferStatusRequest()
        ret = smb.GetTransferStatusResponse()
        while ret[3] != read_length:
            smb.TransferStatusRequest()
            ret = smb.GetTransferStatusResponse()
        smb.ForceReadResponse()
        rev_str = ""

        rev_buf = smb.GetReadResponse()
        for item in range(read_length):
            rev_char = "{:02X} ".format(rev_buf[item])
            rev_str += rev_char
        """
        while rev_len < read_length:
            rev_buf = smb.GetReadResponse()
            rev_len1 = len(rev_buf)
            for item in range(rev_len1):
                rev_str += str(rev_buf[item])
            rev_len += rev_len1
        """
        print(rev_str)
        rc = 0
