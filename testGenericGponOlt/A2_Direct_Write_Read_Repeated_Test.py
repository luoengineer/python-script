...
f.write("\nA2 Direct raw data: \n")
Res = 0xFF
Res = _ateapidll.AteIicRandomRead(devUsbIndex, devSffChannel, SfpI2cAddr[1], 0, 96, A2RawReadByte)
if 0 == Res:
...
for times in range(wr_and_rd_times):
    ...
    f.write('\n')
    _ateapidll.AteIicRandomWrite(devUsbIndex, devSffChannel, SfpI2cAddr[1], 0, 96, byref(A2WriteByte))
    time.sleep(1)
    ...
    _ateapidll.AteIicRandomRead(devUsbIndex, devSffChannel, SfpI2cAddr[1], 0, 96, randomReadByte)
...
testEvb.Sfp_Factory_Pwd_Entry(user_password_type)
time.sleep(1)
_ateapidll.AteIicRandomWrite(devUsbIndex, devSffChannel, SfpI2cAddr[1], 0, 96, byref(A2RawReadByte))
time.sleep(1)
A2ReadDataBuff = ctypes.c_ubyte * 96
randomReadByte = A2ReadDataBuff()
_ateapidll.AteIicRandomRead(devUsbIndex, devSffChannel, SfpI2cAddr[1], 0, 96, randomReadByte)
