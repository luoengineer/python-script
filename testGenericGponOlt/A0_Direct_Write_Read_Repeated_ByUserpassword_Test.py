...
def Sfp_User_Pwd_Entry(userCode):
    ...
    if 351 == userCode:
        factoryPwd = i2cWriteBuf(0x46, 0x54, 0x45, 0x31)
    elif 1 == userCode:
...
time.sleep(1)
print("\n****************************************************************************")
print("A0 Direct Write and Read By User Password stress test, start time : {}".format(dateTime))
print("****************************************************************************")
f.write("\n****************************************************************************")
f.write("\nA0 Direct Write and Read By User Password stress test, start time : {}".format(dateTime))
f.write("\n****************************************************************************")
...
for times in range(wr_and_rd_times):
    testEvb.AteAllPowerOn()
    time.sleep(2)
    testEvb.Sfp_User_Pwd_Entry(userCode)
    time.sleep(1)
...
if wr_and_rd_times == totalSuccess:
    print('A0 Direct write and read data By User Password {} times PASS !'.format(wr_and_rd_times))
    f.write('A0 Direct write and read data {} times PASS !'.format(wr_and_rd_times))
else:
    print('A0 Direct write and read data By User Password {} times FAIL !'.format(wr_and_rd_times))
...
dateTime = "{:4}-{:02}-{:02} {:02}:{:02}:{:02}".format(dateTime.tm_year,dateTime.tm_mon,dateTime.tm_mday,dateTime.tm_hour,dateTime.tm_min,dateTime.tm_sec)
print("\n****************************************************************************")
print("A0 Direct Write and Read By User Password stress test, end time : {}, elapsed time : {:2d} h {:2d} m {:.02f} s".format(dateTime, int(time.time()-startTick)//3600,int(time.time()-startTick)%3600//60,int(time.time()-startTick)%3600%60))
print("****************************************************************************")
f.write("\n****************************************************************************")
f.write("\nA0 Direct Write and Read By User Password stress test, end time : {}, elapsed time : {:2d} h {:2d} m {:.02f} s".format(dateTime, int(time.time()-startTick)//3600,int(time.time()-startTick)%3600//60,int(time.time()-startTick)%3600%60))
f.write("\n****************************************************************************")
testEvb.AteAllPowerOff()
