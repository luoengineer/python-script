...
class Driver_UX3320(Driver):
    ...
    def cmd_write_lut(self, lookupTable):
        ...
        _cmdservdll.SuperCmdSer(strCmdIn, strCmdOut)
...
driveInstance.apc_table = 2, "0x4B,0x00,0x00,0x00,0x58,0xFF,0x00,0x00,0x00,0x58", 512, 1
...
if 'OK' == driveInstance.cmd_write_lut(driveInstance.apc_table):
    ...
    tempIndex = testEvb.adc02TempIndex(strCmdOut)
...
#########################################################
#Backup lut
print("\n\nBackup {} LUT".format(driveInstance.mod_reg[2]))
f.write("\n\nBackup {} LUT".format(driveInstance.mod_reg[2]))
Res = 0xFF
Res, RawReadByte = driveInstance.cmd_read_lut(driveInstance.mod_table[0])
if "OK" == Res:
    print('\n{} LUT :'.format(driveInstance.mod_reg[2]))
    f.write('\n{} LUT :'.format(driveInstance.mod_reg[2]))
    for item in range(len(RawReadByte)):
        print("{}".format(chr(RawReadByte[item])), end='')
        f.write(chr(RawReadByte[item]))
else:
    f.write('read {} LUT fail.'.format(driveInstance.mod_reg[2]))
    print('read {} LUT fail.'.format(driveInstance.mod_reg[2]))
    f.close()
    sys.exit()
ModRawLut = str(RawReadByte, 'utf-8')
...
#Restore lut
print("\nRestore {} Lut".format(driveInstance.mod_reg[2]), end='')
f.write("\nRestore {} Lut".format(driveInstance.mod_reg[2]))
driveInstance.mod_table = 3, ModRawLut, 512, 1
if 'OK' == driveInstance.cmd_write_lut(driveInstance.mod_table):
    print("\n{} LUT restore success.".format(driveInstance.mod_reg[2]))
    f.write("\n{} LUT restore success.".format(driveInstance.mod_reg[2]))
else:
    print("\n{} LUT restore fail.".format(driveInstance.mod_reg[2]))
    f.write("\n{} LUT restore fail.".format(driveInstance.mod_reg[2]))
