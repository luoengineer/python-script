...
Res = 0xFF
Res = objdll.AteIicRandomRead(devUsbIndex, devSffChannel, ComboSfpI2cAddr[2], 0, 128, B0RawReadByte)
if 0 == Res:
...
for times in range(wr_and_rd_times):
    ...
    f.write('\n')
    objdll.AteIicRandomWrite(devUsbIndex, devSffChannel, ComboSfpI2cAddr[2], 0, 128, byref(B0WriteByte))
    time.sleep(1)
    ...
    objdll.AteIicRandomRead(devUsbIndex, devSffChannel, ComboSfpI2cAddr[2], 0, 128, randomReadByte)
...
Sfp_Factory_Pwd_Entry(user_password_type)
time.sleep(1)
objdll.AteIicRandomWrite(devUsbIndex, devSffChannel, ComboSfpI2cAddr[2], 0, 128, byref(B0RawReadByte))
time.sleep(1)
B0ReadDataBuff = ctypes.c_ubyte * 128
randomReadByte = B0ReadDataBuff()
objdll.AteIicRandomRead(devUsbIndex, devSffChannel, ComboSfpI2cAddr[2], 0, 128, randomReadByte)
