...
tx_soft_dis = 0x40
tx_soft_en  = 0x00
txhwdis_to_txsoftdis = 0x40  # A2[118]=0x40, it is RSSI and only for software disable or enable
txsoftdis_to_txhwdis = 0x00  # A2[118]=0x00, it is TX_Disable and only for hardware disable or enable
...
def txSoftDis():
    ...
    # first write A2[118] , software disable or enable active
    i2cWriteByte = i2cWriteBuf(txhwdis_to_txsoftdis)
    objdll.AteIicRandomWrite(devUsbIndex, devSffChannel, ComboSfpI2cAddr[1], 0x76, 1, i2cWriteByte)
    # then write A2[110], disable XGS
    i2cWriteByte = i2cWriteBuf(tx_soft_dis)
    objdll.AteIicRandomWrite(devUsbIndex, devSffChannel, ComboSfpI2cAddr[1], 0x6E, 1, i2cWriteByte)
    # now write B2[110], disable GPON
    i2cWriteByte = i2cWriteBuf(tx_soft_dis)
    objdll.AteIicRandomWrite(devUsbIndex, devSffChannel, ComboSfpI2cAddr[3], 0x6E, 1, i2cWriteByte)
...
def getXGSTxpower():
    ...
    objdll.AteIicRandomRead(devUsbIndex, devSffChannel, ComboSfpI2cAddr[1], 0x66, 2, i2cReadByte)
    return i2cReadByte

def getGPONTxpower():
    i2cReadBuf = ctypes.c_ubyte*2
    i2cReadByte = i2cReadBuf()
    objdll.AteIicRandomRead(devUsbIndex, devSffChannel, ComboSfpI2cAddr[3], 0x66, 2, i2cReadByte)
    return i2cReadByte
