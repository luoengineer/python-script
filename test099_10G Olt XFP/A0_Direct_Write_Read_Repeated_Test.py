...
#               Entry Password
#########################################################
Xfp_Factory_Pwd_Entry(user_password_type)
time.sleep(1)
#########################################################
#               Command Sevices
#########################################################
...
A0RawDataBuff = ctypes.c_ubyte*80
A0RawReadByte = A0RawDataBuff()
f.write("\nA0 Direct raw data: \n")
Res = 0xFF
Res = testEvb.objdll.AteIicRandomRead(devUsbIndex, devSffChannel, XfpI2cAddr[0], 0, 80, A0RawReadByte)
if 0 == Res:
...
for times in range(wr_and_rd_times):
    ...
    A0WriteDataBuff = [0x00] * 80
    A0WriteDataBuff = random_int_list(0, 256, 80)
    A0WriteByte = (c_ubyte * 80)(*A0WriteDataBuff)

    f.write('write :\n')
    for item in range(80):
        f.write(str(hex(A0WriteByte[item]))+',')
    f.write('\n')
    testEvb.objdll.AteIicRandomWrite(devUsbIndex, devSffChannel, XfpI2cAddr[0], 0, 80, byref(A0WriteByte))
    time.sleep(1)
    ...
    A0ReadDataBuff = ctypes.c_ubyte * 80
    randomReadByte = A0ReadDataBuff()
    testEvb.objdll.AteIicRandomRead(devUsbIndex, devSffChannel, XfpI2cAddr[0], 0, 80, randomReadByte)
...
if wr_and_rd_times == totalSuccess:
    ...
    f_report.write('A0 Direct write and read data {} times PASS !'.format(wr_and_rd_times))
else:
    ...
    f_report.write('A0 Direct write and read data {} times FAIL !'.format(wr_and_rd_times))
f.write('\n')
f_report.write('\n')
...
Xfp_Factory_Pwd_Entry(user_password_type)
time.sleep(1)
testEvb.objdll.AteIicRandomWrite(devUsbIndex, devSffChannel, XfpI2cAddr[0], 0, 80, byref(A0RawReadByte))
time.sleep(1)
A0ReadDataBuff = ctypes.c_ubyte * 80
randomReadByte = A0ReadDataBuff()
testEvb.objdll.AteIicRandomRead(devUsbIndex, devSffChannel, XfpI2cAddr[0], 0, 80, randomReadByte)
