...
for times in range(wr_and_rd_times):
    ...
    if wr_and_rd_success == 96:
        totalSuccess += 1
        f.write('Round.{}: B2 write data equal read data, OK.'.format(times)+'\n\n')
        print("Round.{} B2 write data equal read data, OK.".format(times))
    else:
        f.write('Round.{}: B2 write data not equal read data, FAIL.'.format(times)+'\n\n')
        print('Round.{}: B2 write data not equal read data, FAIL.'.format(times)+'\n\n')
...
if wr_and_rd_times == totalSuccess:
    ...
    f_report.write('B2 Direct write and read data {} times PASS !'.format(wr_and_rd_times))
else:
    ...
    f_report.write('B2 Direct write and read data {} times FAIL !'.format(wr_and_rd_times))
f.write('\n')
f_report.write('\n')
