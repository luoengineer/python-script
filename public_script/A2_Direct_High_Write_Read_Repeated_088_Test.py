...
for times in range(wr_and_rd_times):
    ...
    if wr_and_rd_success == 116:
        totalSuccess += 1
        f.write('Round.{}: A2 Direct High write data equal read data, OK'.format(times)+'\n\n')
        print("Round.{} A2 Direct High write data equal read data, OK".format(times))
    else:
        f.write('Round.{}: A2 Direct High write data not equal read data, FAIL'.format(times)+'\n\n')
        print('Round.{}: A2 Direct High write data not equal read data, FAIL'.format(times)+'\n\n')
